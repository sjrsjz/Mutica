...
impl<T: GcAllocObject<T, Inner = Type<T>>> Sequence<T> {
    ...
    // --- 辅助方法：获取有限长度 ---
    fn get_finite_len(&self) -> Result<usize, TypeError<Type<T>, T>> {
        match &self.ty {
            SequenceType::Unit => Ok(0),
            SequenceType::NonEmptyTuple(_) => {
                let total_len = self.physical_prefix_len();
                if self.offset > total_len {
                    Err(TypeError::RuntimeError(Arc::new(std::io::Error::new(
                        std::io::ErrorKind::InvalidData,
                        "Sequence offset exceeds its length",
                    ))))
                } else {
                    Ok(total_len - self.offset)
                }
            }
            _ => Err(TypeError::TypeMismatch(
                (self.as_ref_dispatcher().clone_data(), "Finite Sequence (Tuple or Unit)".into())
                    .into(),
            )),
        }
    }
    ...
    fn truncate_to<'a>(
        ...
        let mut new_prefix: Vec<(Type<T>, NonZero<usize>)> = Vec::new();
        let mut collected = 0;
        ...
        if let Some((start_idx, start_rem)) = self.seek_prefix() {
            ...
            if collected < target_len {
                // 2. 处理后续块
                for (ty, count) in self.physical_prefix().iter().skip(start_idx + 1) {
                    let cnt = count.get();
        ...
        // 如果循环完了 collected < target_len，说明 self 长度不够，或者 self 是空的
        // 这种情况下，我们只返回能拿到的部分（类似 take 语义），还是报错？
        // 通常数学运算要求严格，但作为 Slice 语义，取 min(len, target) 是合理的。
        // 上面的逻辑自然实现了 min(len, target)。

        let new_prefix_arc: Arc<[_]> = Arc::from(new_prefix);

        Ok(Sequence {
            ty: if new_prefix_arc.is_empty() {
                SequenceType::Unit
            } else {
                SequenceType::NonEmptyTuple(new_prefix_arc)
            },
    ...
    pub fn sub<'a>(
        ...
        // 检查下溢
        let target_len = len_self.checked_sub(len_other).ok_or_else(|| {
            TypeError::RuntimeError(Arc::new(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "Sequence length underflow during subtraction",
            )))
        })?;
    ...
    pub fn mul<'a>(
        ...
        if let Some((start_idx, start_rem)) = self.seek_prefix() {
            template.push((&self.physical_prefix()[start_idx].0, start_rem));
            for (ty, count) in self.physical_prefix().iter().skip(start_idx + 1) {
                template.push((ty, count.get()));
            }
        } else {
            // Self 是空的
            ...
            match new_prefix.last_mut() {
                    ...
                    ) =>
                {
                    ...
                    let new_count = current.checked_add(count).ok_or_else(|| {
                        TypeError::RuntimeError(Arc::new(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            "Sequence length overflow during multiplication",
                        )))
                    })?;
                    *last_count = unsafe { NonZero::new_unchecked(new_count) };
                }
