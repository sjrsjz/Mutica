...
pub struct CharacterValue<U: CoinductiveType<U, V>, V: GcAllocObject<V>> {
    value: char,
    source_info: Option<Arc<SourceLocation>>,
    _phantom: std::marker::PhantomData<(U, V)>,
}

impl<U: CoinductiveType<U, V>, V: GcAllocObject<V>> Clone for CharacterValue<U, V> {
    fn clone(&self) -> Self {
        Self {
            value: self.value,
...
impl<U: CoinductiveType<U, V>, V: GcAllocObject<V>> GCTraceable<V> for CharacterValue<U, V> {
    fn collect(&self, _queue: &mut std::collections::VecDeque<arc_gc::arc::GCArcWeak<V>>) {}
}

impl<U: CoinductiveType<U, V>, V: GcAllocObject<V>> Rootable<V> for CharacterValue<U, V> {
    fn rootless(&self) -> bool {
        true
    }
}

impl<T: GcAllocObject<T, Inner = Type<T>>> AsDispatcher<Type<T>, T> for CharacterValue<Type<T>, T> {
...
impl<T: GcAllocObject<T, Inner = Type<T>>> Representable for CharacterValue<Type<T>, T> {
    fn represent(
        &self,
...
impl<T: GcAllocObject<T, Inner = Type<T>>> CharacterValue<Type<T>, T> {
    #[allow(clippy::new_ret_no_self)]
    pub fn new(value: char, source_info: Option<Arc<SourceLocation>>) -> Type<T> {
        CharacterValue::<Type<T>, T> { value, source_info, _phantom: std::marker::PhantomData }
            .dispatch()
    }
