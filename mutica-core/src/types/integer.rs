...
use crate::{
    types::{
        ...
        TypeRef, integer_value::IntegerValue,
    },
    util::{
...
pub struct Integer<T: GcAllocObject<T, Inner = Type<T>>> {
    ...
    fn report_source_info(&self) -> crate::types::TypeReport {
        if let Some(loc) = &self.source_info {
            ...
            ariadne::Report::build(ariadne::ReportKind::Error, filepath.clone(), span.start)
                .with_message(format!("Type 'Integer' at {}", filepath))
                .with_label(
                    ariadne::Label::new((filepath, span)).with_message("Integer type defined here"),
                )
                .finish()
        } else {
            ariadne::Report::build(ariadne::ReportKind::Error, "<unknown>".to_string(), 0)
                .with_message("Type 'Integer' has no source location")
                .with_label(
